...
/** Persist an account object to localStorage */
const setStoredAccount = (profile) => {
  localStorage.setItem("account", JSON.stringify(profile));
};

/** Remove the persisted account */
    ...
    <div className="stars">
      {[1, 2, 3, 4, 5].map((s) => (
        <span
          key={s}
          className={
            s <= Math.floor(rating) ? "star filled" : s - 0.5 <= rating ? "star half" : "star"
          }
        >
          ★
        </span>
...
export default function App() {
  ...
  // FIX 2: consistent API_BASE fallback in handleLogin
  const handleLogin = async (event) => {
    ...
    try {
      ...
      setUser(profile);
      setStoredAccount(profile); // persist session
      if (data.address) {
  ...
  const handleRegister = async (event) => {
    ...
    try {
      ...
      // FIX 3: setStoredAccount is now defined above — no more ReferenceError
  ...
  const handleAddressUpdate = (event) => {
    ...
    setUser(updatedUser);
    // FIX 3 (same fix): setStoredAccount is now defined
      ...
      <nav className="navbar" style={{ position: "relative", zIndex: 100 }}>
        ...
        <div className="nav-actions" style={{ position: "relative", zIndex: 100 }}>
          ...
          <button
            className="cart-btn"
            style={{ cursor: "pointer", position: "relative", zIndex: 100 }}
            onClick={() => { setCheckoutStep("summary"); setCartOpen(true); }}
          >
            🛒 Cart
      ...
      {cartOpen && (
        ...
        >
          <div
            ...
            style={{ position: "fixed", right: 0, top: 0, bottom: 0, width: 400, background: "#fff", overflowY: "auto", padding: 20, zIndex: 1001 }}
        ...
        >
          ...
          >
            ...
            {checkoutStep === "summary" ? (
              ...
              /* FIX 4: replaced <form> with <div> + onClick handlers */
              <div className="checkout-form">
                ...
                <div className="checkout-section">
                  ...
                  <label>Cardholder Name<input type="text" value={paymentDetails.billingName} onChange={(e) => setPaymentDetails((p) => ({ ...p, billingName: e.target.value }))} /></label>
                  <label>Card Number<input type="text" value={paymentDetails.cardNumber} onChange={(e) => setPaymentDetails((p) => ({ ...p, cardNumber: e.target.value }))} /></label>
                  <label>Expiry<input type="text" placeholder="MM/YY" value={paymentDetails.expiry} onChange={(e) => setPaymentDetails((p) => ({ ...p, expiry: e.target.value }))} /></label>
                  <label>CVV<input type="password" value={paymentDetails.cvv} onChange={(e) => setPaymentDetails((p) => ({ ...p, cvv: e.target.value }))} /></label>
                </div>
                <div className="cart-footer">
                  <div className="cart-total"><span>Order Total:</span><strong>₹{cartTotal.toLocaleString()}</strong></div>
                  <button className="checkout-btn" onClick={handlePlaceOrder}>Pay Now</button>
                  <button className="continue-btn" onClick={() => setCheckoutStep("summary")}>Back to Summary</button>
                </div>
              </div>
            )}
          </div>
          ...
          >
            ...
            {user ? (
              <div className="account-section">
                ...
                {/* FIX 4: replaced <form> with <div> + onClick */}
                <div className="checkout-form">
                  ...
                  <button className="checkout-btn" onClick={handleAddressUpdate}>Save Address</button>
                </div>
                {orders.length > 0 && (
                  <>
                    <hr />
